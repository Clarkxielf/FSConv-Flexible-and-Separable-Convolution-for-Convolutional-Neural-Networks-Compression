...
import Visualization.ResNet20_on_CIFAR10_Baseline as ResNet20_on_CIFAR10
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet20',
                    ...
                    ' (default: resnet20)')
parser.add_argument('--resume', default='../Visualization/tabby_cat_s_000319/checkpointResNet20_CIFAR10/model_best.pth.tar', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: ../Visualization/tabby_cat_s_000319/checkpointResNet20_CIFAR10/model_best.pth.tar)')
...
def main():
    ...
    for Img in range(img.shape[0]):
        Baseline = img[Img]
        Baseline = np.repeat(Baseline[None, :, :], img.shape[0], axis=0)
        MSE_Baseline = np.mean((Baseline - img) ** 2, axis=(1, 2))

        if len(str(Img)) == 1:
            print("MSE_Baseline_0{}:{}".format(Img+1, MSE_Baseline))

            Colorful_Matrix.append(MSE_Baseline)
        else:
            print("MSE_Baseline_{}:{}".format(Img+1, MSE_Baseline))
    ...
    plt.savefig(r'./tabby_cat_s_000319_Baseline.png')
    plt.show()
