...
import Baseline.VGG16_on_CIFAR10 as VGG16_on_CIFAR10


model_names = sorted(name for name in VGG16_on_CIFAR10.__dict__
                     if name.islower() and not name.startswith("__")
                     and name.startswith("vgg")
                     and callable(VGG16_on_CIFAR10.__dict__[name]))
...
parser.add_argument('--resume', default='./checkpointVGG16_CIFAR10/model_best.pth.tar', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: ./checkpointVGG16_CIFAR10/model_best.pth.tar)')
...
def main():
    ...
    model = VGG16_on_CIFAR10.__dict__[args.arch]()
    print_log("=> network :\n {}".format(model), log)
