...
import FSConv.ResNet20_on_CIFAR10_FSConv as ResNet20_on_CIFAR10_FSConv

model_names = sorted(name for name in ResNet20_on_CIFAR10_FSConv.__dict__
                     if name.islower() and not name.startswith("__")
                     and name.startswith("resnet")
                     and callable(ResNet20_on_CIFAR10_FSConv.__dict__[name]))
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet20',
                    ...
                    ' (default: resnet20)')
parser.add_argument('--Alpha', default=0.7, type=float, metavar='A',
                    help='Alpha')
parser.add_argument('--Beta', default=2, type=float, metavar='A',
                    help='Beta')
parser.add_argument('--save-dir', dest='save_dir',
                    help='The directory used to save the trained models',
                    default='./checkpointResNet20', type=str)
parser.add_argument('-e', '--evaluate', dest='evaluate', action='store_true',
                    default=True,
                    help='evaluate model on validation set')
parser.add_argument('--resume', default='./checkpointResNet20_CIFAR10_0.7_2/model_best.pth.tar', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: ./checkpointResNet20_CIFAR10_0.7_2/model_best.pth.tar)')
...
def main():
    ...
    model = ResNet20_on_CIFAR10_FSConv.__dict__[args.arch]()
    print_log("=> network :\n {}".format(model), log)
    ...
    if args.evaluate:
        ...
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                            milestones=[75, 150], last_epoch=args.start_epoch - 1)
